...
scennumb = as.numeric(commandArgs(trailingOnly = T)[1])


scennum = floor(scennumb/100)
b = scennumb %% 100



scens <- read_csv("Data/sensAnalysis_scenarios.csv")
...
experiment_name = "posneg Inflect beta2"

seirInflect@params <- scens %>% 
  filter(scenario_number == scennum) %>% 
  select(-scenario_number, -scenario_name) %>% 
  unlist
...
# b = 1
  ...
  start_tinit_vec = seq(as.Date("2020-02-17"), as.Date("2020-03-12"), by = 1)  
  

  for(i in 1:NTESTS){
    
    set.seed(scennumb*100 + i)
    start_tinit = sample(start_tinit_vec, 1)
    ...
    start_tinflect = seirInflect@params["t_inflect"]
    
    # start_Einit = j #max(1, round(1/runif(1)))
    
    print(i)
    print(c(start_beta1, start_beta2, start_tinit))
    
    updated_seed = scennumb^2*100 + i
    set.seed(updated_seed)
    
    seirInflect %>% 
      mif2(params = seirInflect@params %>% replace(c("beta1", "beta2", "t_init"), c(start_beta1, start_beta2, start_tinit))
    ...
    res %>% write_csv(paste0("TempResults/sensAnal_seirInflect_", experiment_name, "_scennum", scennum, "_startbeta1", start_beta1 , ".csv"))
