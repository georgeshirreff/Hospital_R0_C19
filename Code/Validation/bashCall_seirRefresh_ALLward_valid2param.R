...
aw = as.numeric(commandArgs(trailingOnly = T)[1])
#0 to nrow(valid table)*10 + 9
# aw = 980
a = floor(aw / 10)
w = aw %% 10
...
# valid_table <- read_csv("input/validation/seirRefresh_validation_2param_tinit.csv")
valid_table <- read_csv("Data/BiasTest_2param.csv")
posneg = read_csv(paste0("input/posneg_alltests_", wardName, ".csv")) %>%
...
posneg %>% 
  pomp(t0 = posneg$Date[1]
       , times="Date"
       , params=c(beta = 0.3 # frequency-dependent transmission rate from Is (symptomatic infection)
       ...
       , rprocess=gillespie_hl(
         ...
         , StoE = list("rate = S*beta*(epsilon*(Es + EsT + kappa1*(Ea + EaT)) + (Is + IsT + kappa1*(Ia + IaT)))/N;"
         ...
         , STtoET = list("rate = ST*beta*(epsilon*(Es + EsT + kappa1*(Ea + EaT)) + (Is + IsT + kappa1*(Ia + IaT)))/N;"
...
experiment_name = "posnegWard Refresh valid2param tinit extinction_threshold0"
...
# proposal_call = c("rw.sd(beta1 = 0.02, beta_factor = 0.02, E_init = ivp(1), t_init = ivp(0), t_inflect = ivp(20))")
# proposal_call = c("rw.sd(beta1 = ifelse(time < start_tinflect, 0.02, 0), beta2 = ifelse(time >= start_tinflect, 0.02, 0), t_init=ivp(1))")
proposal_call = c("rw.sd(beta = 0.02, t_init = ivp(1))")
...
# i = 1
  ...
  for(i in 1:NTESTS){
    ...
    orig_seed = w^3 + a*1000 + i
    updated_seed = orig_seed
    
    set.seed(orig_seed)
    
    sims <- seirRefresh %>% simulate(nsim = 1
                                 , include.data = F
                                 , params = seirRefresh@params %>% replace(names(true_parameters), true_parameters))
    ...
    sims %>% 
      mif2(params = seirRefresh@params %>% replace(names(start_parameters), start_parameters)
           , Np=mif2_Np, Nmif=Nmif
           # , Np = 10, Nmif = 100 #seems fine
           # , partrans=parameter_trans(log=c(estimate_parameter))
           # , partrans=parameter_trans(log=c(transform_parameter))
           , partrans = parameter_trans(toEst = Csnippet("\tT_beta = beta;")
                                        , fromEst = Csnippet("\tbeta = fabs(T_beta);"))
    ...
    res_piece <- toy_mif %>% coef() %>% bind_rows() %>%
      bind_cols(loglik=ll[1],loglik.se=ll[2], rep = i, valid_table_row, wardCode = wardCode, wardName = wardName, max_pos = max_pos, max_Is = max_Is, max_SAR_numer = max_SAR_numer)
    ...
    res %>% write_csv(paste0("TempResults/BiasTest_seirRefresh_", experiment_name, "_Array", a, "_w", wardCode, ".csv"))
