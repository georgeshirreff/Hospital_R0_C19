...
wardName = paste0(case_when(wardNum %in% 1:4 ~ "A"
                            , wardNum %in% 5:8 ~ "B"
                            , wardNum %in% 9:12 ~ "C")
                  , "_",  ((wardNum-1) %% 4))
...
posneg = read_csv(paste0("Data/posneg_alltests","_", wardName,".csv")) %>%
  ...
  for(i in 1:NTESTS){
    ...
    seirRefresh %>% 
      mif2(params = seirRefresh@params %>% replace(c("beta", "t_init", "E_init"), c(start_beta, start_tinit, start_Einit))
           ...
           , paramnames=c("t_init", "beta")
           , cooling.fraction.50=cooling.fraction
           , rw.sd=rw.sd(beta = 0.02, t_init=ivp(1))
      ) -> mif_out
    

    set.seed(updated_seed*100)
    ...
    res_piece <- mif_out %>% coef() %>% bind_rows() %>%
      bind_cols(loglik=ll[1],loglik.se=ll[2], rep = i, seed = updated_seed, wardName = wardName, wardCode = wardCode, start_beta = start_beta, start_Einit = start_Einit, start_tinit = start_tinit)
    ...
    res %>% write_csv(paste0("TempResults/seirRefresh_", experiment_name,"_", wardCode, "_startbeta", start_beta, "_starttinit", start_tinit , ".csv"))
