...
# # R0 calculation
# seirRefresh@params["beta"]/seirRefresh@params["delta"]
...
experiment_name = "posneg betaProfile tinit Einit1 seedFix"
...
# b = 1
  ...
  start_beta = betavec[b]
  start_Einit = 1
  start_tinit = posneg$Date[j]
  
  print(c(start_beta, start_Einit, start_tinit))
  
  # res <- read_csv(paste0("output/seirRefresh_", experiment_name, "_startbeta", start_beta, "_starttinit", start_tinit , ".csv"))
  # trace <- read_csv(paste0("output/seirRefresh_", experiment_name, "_startbeta", start_beta, "_starttinit", start_tinit , "_trace.csv"))
  
  # if(max(res$rep) < NTESTS){
  # start_i = max(res$rep) + 1
  # for(i in start_i:NTESTS){
  ...
  for(i in 1:NTESTS){
    ...
    seirRefresh %>% 
      mif2(params = seirRefresh@params %>% replace(c("beta", "t_init", "E_init"), c(start_beta, start_tinit, start_Einit))
           , Np=mif2_Np, Nmif=Nmif #Np = 10, Nmif = 100 seems fine
           # , partrans=parameter_trans(log=c("beta"))
           , paramnames=c("t_init")
           , cooling.fraction.50=cooling.fraction
           , rw.sd=rw.sd(t_init=ivp(2))
      ) -> mif_out
    ...
    res_piece <- mif_out %>% coef() %>% bind_rows() %>%
      bind_cols(loglik=ll[1],loglik.se=ll[2], rep = i, seed = updated_seed, start_beta = start_beta, start_Einit = start_Einit, start_tinit = start_tinit)
    
    # trace_piece <- mif_out %>% traces %>% 
    #   as_tibble %>% 
    #   transmute(iteration = 0:(n()-1)
    #             , loglik, rep = i, seed = updated_seed, beta, E_init, t_init, start_beta = start_beta, start_Einit = start_Einit, start_tinit = start_tinit)
    ...
    res %>% write_csv(paste0("TempResults/seirRefresh_", experiment_name, "_startbeta", start_beta, "_starttinit", start_tinit , ".csv"))
    # trace %>% write_csv(paste0("output/seirRefresh_", experiment_name, "_startbeta", start_beta, "_starttinit", start_tinit , "_trace.csv"))
